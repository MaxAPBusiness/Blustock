...
# Función mostrar datos: busca los datos de la tabla de la base de datos y los muestra en la tabla con la que el usuario puede interactuar. Parámetro:
    ...
    def modificarLinea(self, tipo):
        ...
        self.edita.setWindowTitle("Agregar Profesor")
    ...
    def confirmarModificacion(self, tipo, datosPorDefecto=None):
        if len(self.entry2.text()) > 50:
            m.mostrarMensaje("Error", "Error", "El nombre ingresado es demasiado largo. Ingresa uno más corto.")
            return
        elif len(self.entry4.text()) > 100:
            m.mostrarMensaje("Error", "Error", "El email ingresado es demasiado largo. Ingresa uno más corto.")
        datosNuevos=(
