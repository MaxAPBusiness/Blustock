...
------
    GestionHerramientas:
        Crea una pantalla para gestionar la tabla de herramientas.
"""
...
class GestionHerramientas(qtw.QWidget):
    ...
    ---------
        ...
        radioGrupo : QRadioButton
            el botón de radio para ordenar por grupo.
        radioSubgrupo : QRadioButton
            el botón de radio para ordenar por subgrupo.
        ...
        __init__(self):
            ...
            Crea la pantalla, un QWidget, que contiene:
                - Una tabla, un QTableWidget, que muestra los datos de
                  la tabla herramientas y contiene botones para editarlos.
            ...
            Ver también
            -----------
            mostrarDatos: obtiene los datos de la tabla herramientas y los
    ...
    def __init__(self):
        ...
        contenedor1Layout.addWidget(labelOrdenar, 1, 1)
        contenedor1Layout.addWidget(self.radioNombre, 1, 2)
        contenedor1Layout.addWidget(self.radioGrupo, 1, 3)
        contenedor1Layout.addWidget(self.radioSubgrupo, 1, 4)
    ...
    def modificarLinea(self, tipo):
        ...
        self.entry6.textEdited.connect(lambda:self.cargarSubgrupos(self.entry5.text()))
        ...
        if tipo == "editar":
            ...
            self.entry6.setText(datos[5])
            self.entry7.setText(datos[6])
    ...
    def cargarSubgrupos(self, grupo):
        """Esta función rellena el cuadro de sugerencias de subgrupos.
    ...
    def confirmarModificacion(self, tipo, datosPorDefecto=None):
        ...
        intenta realizar los cambios, registrarlos en el historial y
        notificar al usuario el éxito de la operacion. Si la base de
        ...
        datosNuevos=(
            ...
            self.entry4.value(), self.entry5.value(), self.entry6.text(), 
            self.entry7.text()
            )

        if tipo=="editar":
            try:
                db.cur.execute("SELECT * FROM herramientasWHERE ID = ?", (datosPorDefecto[0]))
                datosViejos=db.cur.fetchall()[0]
                db.cur.execute("""
                UPDATE herramientas
                SET ID = ?, desc_larga = ?, cant_condiciones = ?, 
                cant_reparacion = ?, cant_baja = ?, total = ?,
                grupo = ?, subgrupo = ?
                WHERE ID = ?""", (
                    datosNuevos[0], datosNuevos[1], datosNuevos[2], datosNuevos[3],
                    datosNuevos[4], datosNuevos[5], datosNuevos[6],  datosPorDefecto,
                ))
                registrarCambios("Edición", "Herramientas", datosPorDefecto[0], f"{datosViejos}", f"{datosNuevos}")
                db.con.commit()
            ...
            try:
                db.cur.execute("INSERT INTO herramientasVALUES(?, ?, ?, ?, ?, ?, ?) ", datosNuevos)
    ...
    def eliminar(self):
        """Esta función elimina la fila de la tabla herramientas.
